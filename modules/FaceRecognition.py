                ...
                for face_encoding in face_encodings:
                    ...
                    if len(face_distances) != 0:
                        ...
                        if matches[best_match_index]:
                            ...
                            if name not in greeted_people:
                                modules.TextToSpeech.play("Welcome back " + name, repeat=False)
                                modules.TextToSpeech.play("Can i help you?", repeat=False)
                    ...
                    if name == 'Unknown':
                        ...
                        if temp_stop:
                            ...
                            modules.TextToSpeech.play("What is your name?", repeat=False)
                            while True:
                                name_for_unknown = modules.SpeechToText.play(return_value = True)
                                modules.TextToSpeech.play("Your name is " + name_for_unknown + ', right?', repeat=False)
                                confirm_answer = modules.SpeechToText.play(return_value = True)
                                if (name_for_unknown != '0') and ('yes' in confirm_answer):
                                    global_utils.show_module_log("FR - Now i know you! Nice to meet you.")
                                    modules.TextToSpeech.play("Nice to meet you " + name_for_unknown, repeat=False)
                                    greeted_people.append(name_for_unknown)
                                    break
                                else:
                                    global_utils.show_module_log("FR - Try again!")
                                    modules.TextToSpeech.play("Say your name again", repeat=False)
